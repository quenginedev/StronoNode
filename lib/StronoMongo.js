...
module.exports = class StronoMongo extends StronoComposer{
    ...
    createMutations(composition){
        this.compositions.forEach(composition=>{
            ...
            this.Mutation[`delete${composition.name}`] = this.deleteOne(composition)
                    ...
                    if(data[compositionRef.name]){
                        ...
                        if(compositionRef.many){
                                ...
                                for (let item of createData){
                                    await this.applyAssociation(composition.name, compositionRef.type, async association=>{
                                        ...
                                        if (association.many)
                                            ...
                                            itemModel[association.name].addToSet(data._id)
                                        else
                            ...
                            if(connectData){
                                //Sets associated collection that has been referenced the data id
                                    await this.applyAssociation(composition.name, compositionRef.type, async association=>{
                                        let foundConnection = await this.models[compositionRef.type].findOne({ _id: connectData })
                                        if (foundConnection) {
                                            if (association.many) {
                                                foundConnection[association.name].addToSet(data._id)
                                            } else {
    ...
    deleteOne(composition){
        return {
            type: this.getType(composition.name),
            args: {
                where: { type: new GraphQLNonNull(this.getInputType(`${composition.name}WhereUniqueInput`)) },
                associate: { type: new GraphQLInputObjectType({
                        name: `${composition.name}AssociateType`,
                        fields: ()=>({
            ...
            resolve: async (parent, args)=>{
                let data = await this.models[composition.name].findOne(args.where)
                if(data){
                    let compositionRefs = this.references[composition.name]
                    if (args.associate)
                    for (let compositionRef of compositionRefs) {
                        let associatedRefs = this.references[compositionRef.type]
                        for (let associatedRef of associatedRefs) {
                            if (args.associate.disconnect) {
                                // todo shorten the code
                                let associatedQueries = await this.models[compositionRef.type]
                                    .find({[associatedRef.name]: {$in: {_id: data._id}}})

                                for (let assQuery of associatedQueries) {
                                    if (associatedRef.many)
                                        assQuery[associatedRef.name].pull({_id: data._id})
                                    else
                                        assQuery[associatedRef.name] = null
                                    await assQuery.save()
                                }
                            }
                            if (args.associate.delete) {
                                await this.models[compositionRef.type]
                                    .deleteMany({[associatedRef.name]: {$in: {_id: data._id}}})
                            }

                        }
                        await this.models[composition.name].deleteOne(args.where)
                        return data
                    }
                }
            }
